...
async function processFeedItem({ feedUrl, feedTitle }, item) {
  ...
  try {
    ...
    try {
      ...
      console.error('Failed to persist summarize error to Mongo:', saveErr);
    }
  ...
  for (const doc of pending) {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to persist retry error to Mongo:', saveErr);
      }
...
function startScheduler() {
  cron.schedule(env.FETCH_CRON, () => {
    runOnce().catch((err) => console.error('Job error:', err));
  });
